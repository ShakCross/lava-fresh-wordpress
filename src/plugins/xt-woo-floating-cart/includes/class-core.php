...
class XT_Woo_Floating_Cart {
	...
	private function define_admin_hooks() {
        ...
        if(!$this->fs()->can_use_premium_code__premium_only()) {
            $this->loader->add_filter( 'auto_update_plugin', $this->plugin_admin, 'auto_update', 10, 2 );
        }
	...
	private function define_public_hooks() {
		...
		if($this->fs()->can_use_premium_code__premium_only()) {
			$this->loader->add_action( 'xt_woofc_cart_body_header', $this->plugin_public, 'render_coupon_form__premium_only', 10 );
			$this->loader->add_action( 'xt_woofc_cart_body_footer', $this->plugin_public, 'render_checkout__premium_only', 15 );
			$this->loader->add_action( 'xt_woofc_cart_body_footer', $this->plugin_public, 'render_totals__premium_only', 20 );
			$this->loader->add_filter( 'woocommerce_cart_totals_coupon_html', $this->plugin_public, 'filter_coupon_html__premium_only', 10, 1 );
		}

		$this->loader->add_action( 'init', $this->plugin_public, 'define_woocommerce_constants', 10);
		$this->loader->add_action( 'init', $this->plugin_public, 'init_ajax', 30);
	...
	public function xirki_init() {

        add_filter( 'xirki_telemetry', '__return_false', 1);

        require_once $this->plugin_path() . 'includes/xirki/xirki.php';
		require_once $this->plugin_path() . 'includes/customizer/class-customizer.php';
			
		$this->loader->add_action('after_setup_theme', $this, 'customizer_init');
